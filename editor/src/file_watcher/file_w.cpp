...
FileW::FileW(const std::filesystem::path &path_to_watch,
             std::chrono::duration<int, std::milli> polling_interval)
    : m_path_to_watch(path_to_watch), m_polling_interval(polling_interval),
      m_running(false) {

  const bool directory = std::filesystem::exists(path_to_watch);
  if (!directory) {
    log_error() << "Directory passed to file watcher does not exists:"
                << path_to_watch << std::endl;
    return;
  }
...
FileW::FileW(FileW &&other) noexcept
    : m_path_to_watch(std::move(other.m_path_to_watch)),
      ...
      m_running(other.m_running), m_watch_thread() {
  if (other.m_running) {
...
void FileW::add_callback(const std::vector<std::string> &extensions,
                         Callback callback) {
...
void FileW::watch_loop() {
  while (m_running) {
    ...
    const bool directory_exists =
        std::filesystem::exists(m_path_to_watch); // waiting until the directory
                                                  // we are looking is created
    if (!directory_exists) {
      continue;
    }
...
void FileW::notify_callbacks(const std::string &path,

...
void VariantDocument::load_from_file() {
  ...
  try {
    json_string = std::string(std::istreambuf_iterator<char>(in_file),
                              std::istreambuf_iterator<char>());
  } catch (const std::exception &e) {
    log_error() << "Error: Failed to read variant file: " << path << " - "
                << e.what() << std::endl;
    in_file.close();
    return;
  }
  ...
  if (m_document.HasParseError()) {
    log_error() << "Error: JSON parse error in variant file: " << path
                << std::endl;
  ...
  if (!m_document.IsObject()) {
    log_error() << "Error: Variant file does not contain a valid JSON object: "
                << path << std::endl;
    return;
  }
}

...
TestViewer::TestViewer() { create_mockup_data(); }
...
void TestViewer::render_plan_selection() {
  ...
  if (ImGui::BeginTable("test_plans_table", 3,
                        ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
    ImGui::TableSetupColumn("Test Plan Key", ImGuiTableColumnFlags_WidthFixed,
                            120.0f);
    ImGui::TableSetupColumn("Summary", ImGuiTableColumnFlags_WidthStretch);
    ImGui::TableSetupColumn("Test Count", ImGuiTableColumnFlags_WidthFixed,
                            100.0f);
    ...
    for (int i = 0; i < m_test_plans.size(); i++) {
      ...
      if (ImGui::Selectable(plan.key.c_str(), false,
...
void TestViewer::render_test_execution_view() {
  ...
  ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "| %s",
                     m_test_plans[m_current_plan_index].summary.c_str());
  ImGui::EndGroup();

  ImGui::Separator();

  ImGui::BeginChild("test_summary", ImVec2(0, ImGui::GetTextLineHeight() * 4),
                    true);
  render_summary();
  ImGui::EndChild();
...
void TestViewer::render_summary() {
  float completion =
      m_test_execution.test_runs.empty()
          ? 0.0f
          : (float)m_test_execution.get_executed_test_runs_count() /
                m_test_execution.test_runs.size();
  ...
  ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), "%.0f%%",
                     completion * 100);
  ...
  for (int i = 1; i < static_cast<int>(Status::LENGTH); i++) {
    ...
    ImGui::TextColored(color, "%s: %d", get_status_string(status).c_str(),
                       m_test_execution.get_run_count_with_status(status));
    ImGui::NextColumn();
  }
}
...
void TestViewer::render_test_run(int index) {
  ...
  ImGui::PushStyleColor(ImGuiCol_HeaderHovered,
                        get_status_bg_color(run.status));
  ImGui::PushStyleColor(ImGuiCol_HeaderActive, get_status_bg_color(run.status));

  bool is_open =
      ImGui::CollapsingHeader((run.test_key + ": " + run.test_summary).c_str(),
  ...
  ImGui::TextColored(get_status_color(run.status), "%s",
                     get_status_string(run.status).c_str());

  bool is_header_hovered =
      ImGui::IsMouseHoveringRect(header_start_pos, header_end_pos);
...
void TestViewer::render_test_step(TestRun &run, int step_idx) {
  ...
  ImGui::PushStyleColor(ImGuiCol_HeaderHovered,
                        get_status_bg_color(step.status));
  ImGui::PushStyleColor(ImGuiCol_HeaderActive,
                        get_status_bg_color(step.status));

  bool is_open =
      ImGui::CollapsingHeader(("Step " + std::to_string(step_idx + 1)).c_str());
  ...
  ImGui::SameLine(ImGui::GetContentRegionAvail().x - 100);
  ImGui::TextColored(get_status_color(step.status), "%s",
                     get_status_string(step.status).c_str());
  ...
  if (is_open) {
    ...
    ImGui::BeginChild(("action_text_" + step.id).c_str(),
                      ImVec2(ImGui::GetContentRegionAvail().x - 8,
    ...
    if (ImGui::InputTextMultiline(
            ("##actual_result_" + step.id).c_str(), buffer.data(),
            buffer.size(),
            ImVec2(ImGui::GetContentRegionAvail().x - 8,
                   ImGui::GetTextLineHeightWithSpacing() * 3),
...
void TestViewer::create_mockup_data() {
  ...
  plan1.test_summaries = {"Verify main menu navigation",
  ...
  plan2.test_summaries = {"Verify application startup time",
  ...
  plan3.test_summaries = {"Test input validation and sanitization",
  ...
  m_test_execution.description =
      "Testing the application across different browsers and platforms";
  ...
  step2.result =
      "Each menu item correctly navigates to the corresponding screen";
  ...
  step5.id = "STEP-005";
  step5.action =
      "Close and reopen the application, then open the saved document";
  ...
  step7.result =
      "Components are successfully added and visible in the inspector";
...
ImVec4 get_status_color(Status status) {
  switch (status) {
  case Status::PASSED:
    return ImVec4(0.0f, 0.8f, 0.0f, 1.0f);
  case Status::FAILED:
...
ImVec4 get_status_bg_color(Status status) {
  ImVec4 color = get_status_color(status);
  return ImVec4(color.x * 0.5f + 0.5f, color.y * 0.5f + 0.5f,
                color.z * 0.5f + 0.5f, 0.3f);
}

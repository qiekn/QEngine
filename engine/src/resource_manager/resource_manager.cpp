...
void ResourceManager::construct_paths() {
  ...
  while (level < MAX_LEVELS) {
    bool found_resources =
        std::filesystem::exists(current_dir / SHARED_RESOUCES);
  ...
  // if EDITOR_MODE, resources folder should be provided by editor, if
  // standalone, nothing we can do
...
std::filesystem::directory_iterator ResourceManager::get_variant_folder()
    const {
...
std::filesystem::path ResourceManager::get_entity_path(
    const std::string& name) const {
  return get_entities_path() / (name + ".entity");
}

std::filesystem::path ResourceManager::get_variant_path(
    const std::string& name) const {

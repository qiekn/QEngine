...
inline void draw_line(int startX, int startY, int endX, int endY, Color color) {
...
inline void draw_rectangle(int posX, int posY, int width, int height,
                           Color color) {
...
inline void draw_rectangle_lines(int posX, int posY, int width, int height,
                                 Color color) {
  DrawRectangleLines(posX, posY, width, height, color);
}
inline void draw_rectangle_lines_ex(Rectangle rec, float lineThick,
                                    Color color) {
  DrawRectangleLinesEx(rec, lineThick, color);
}
inline void draw_text(const char* text, int posX, int posY, int fontSize,
                      Color color) {
...
inline void draw_texture_ex(Texture2D texture, Vector2 position, float rotation,
                            float scale, Color tint) {
  DrawTextureEx(texture, position, rotation, scale, tint);
}
inline void draw_texture_pro(Texture2D texture, Rectangle source,
                             Rectangle dest, Vector2 origin, float rotation,
                             Color tint) {
  DrawTexturePro(texture, source, dest, origin, rotation, tint);
}

inline Texture2D load_texture(const char* fileName) {
...
inline Texture2D load_texture_from_image(Image image) {
  return LoadTextureFromImage(image);
}
inline RenderTexture2D load_render_texture(int width, int height) {
...
inline bool check_collision_recs(Rectangle rec1, Rectangle rec2) {
  return CheckCollisionRecs(rec1, rec2);
}
inline bool check_collision_circles(Vector2 center1, float radius1,
                                    Vector2 center2, float radius2) {
  return CheckCollisionCircles(center1, radius1, center2, radius2);
}
inline bool check_collision_circle_rec(Vector2 center, float radius,
                                       Rectangle rec) {
  return CheckCollisionCircleRec(center, radius, rec);
}
inline bool check_collision_point_rec(Vector2 point, Rectangle rec) {
  return CheckCollisionPointRec(point, rec);
}
inline bool check_collision_point_circle(Vector2 point, Vector2 center,
                                         float radius) {
...
inline float vector2_distance_sqr(Vector2 v1, Vector2 v2) {
  return Vector2DistanceSqr(v1, v2);
}
inline float vector2_distance(Vector2 v1, Vector2 v2) {
  return Vector2Distance(v1, v2);
}
inline Vector2 vector2_scale(Vector2 v, float scale) {
  return Vector2Scale(v, scale);
}
inline Vector2 vector2_normalize(Vector2 v) { return Vector2Normalize(v); }

inline Vector2 get_screen_to_world2d(Vector2 position, Camera2D camera) {
  return GetScreenToWorld2D(position, camera);
}
inline Vector2 get_world_to_screen2d(Vector2 position, Camera2D camera) {
  return GetWorldToScreen2D(position, camera);
}

inline bool file_exists(const char* fileName) { return FileExists(fileName); }
inline bool directory_exists(const char* dirPath) {
  return DirectoryExists(dirPath);
}
inline const char* get_file_extension(const char* fileName) {
  return GetFileExtension(fileName);
}
inline const char* get_file_name(const char* filePath) {
  return GetFileName(filePath);
}

inline Color color_alpha(Color c, float alpha) { return ColorAlpha(c, alpha); }
inline Color color_tint(Color color, Color tint) {
  return ColorTint(color, tint);
}
inline Color color_fade(Color color, float alpha) { return Fade(color, alpha); }

inline float get_random_value(int min, int max) {

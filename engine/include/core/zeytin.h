...
public:
  void run_frame();
  inline bool should_die() const {
    return m_should_die || window_should_close();
  }
  ...
  inline const std::unordered_map<entity_id, std::vector<rttr::variant>>&
  get_storage() const {
    return m_storage;
  }
  inline std::unordered_map<entity_id, std::vector<rttr::variant>>&
  get_storage() {
    return m_storage;
  }

...
public:
  using ConfigValue =
      std::variant<int, float, bool, std::string, std::filesystem::path>;
  ...
  T get(const std::string& key, T default_value = T()) const {
    ...
    if (it != m_config_values.end()) {
      try {
        std::cout << "[---------------] Config: " << key
                  << " is found: " << std::get<int>(it->second) << std::endl;
        ;
    ...
    std::cout << "Config: " << key
              << " is not found, returning default value: " << default_value;
    return default_value;
  }
...
private:
  ...
  rapidjson::Value variant_to_json_value(
      const ConfigValue& value,
      rapidjson::Document::AllocatorType& allocator) const;
  ConfigValue json_value_to_variant(const rapidjson::Value& value) const;
};

#define CONFIG_GET(key, type, default_value) \
  ConfigManager::get().get<type>(key, default_value)
